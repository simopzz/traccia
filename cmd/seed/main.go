...
func seedEvents(ctx context.Context, eventService *service.EventService, trip *domain.Trip) (successCount, failureCount int, err error) {
	...
	// Add one transit event per trip
	transitStart := time.Date(
		trip.StartDate.Year(), trip.StartDate.Month(), trip.StartDate.Day(),
		10, 0, 0, 0, trip.StartDate.Location(),
	)
	if err := createTransitEvent(ctx, eventService, trip.ID, transitStart); err != nil {
		slog.Warn("Failed to create transit event", "error", err, "trip_id", trip.ID)
		failureCount++
	} else {
		slog.Debug("Created transit event", "trip_id", trip.ID)
		successCount++
	}
